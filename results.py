...
async def adv_img_save(input_tensor, output_img_name):
    if input_tensor is not None:
        modified_tensor1 = torch.clamp(input_tensor, 0, 1)
        modified_image_pil = transforms.ToPILImage()(modified_tensor1)
        output_path = f"adv_imgs/{output_img_name}"
        await asyncio.to_thread(modified_image_pil.save, output_path)

async def xai_adv_image_save(model, last_layer, input_tensor, img_name):
    ...
    output_path = f"adv_xai_imgs/{img_name}"
    await asyncio.to_thread(gradcam_img.save, output_path)

async def xai_image_save(model, last_layer, input_tensor, img_name):
    with GradCAMpp(model, last_layer) as cam_extractor:
        output = model(input_tensor.unsqueeze(0))
        class_index = output.squeeze(0).argmax().item()
        activation_map = cam_extractor(class_index, output)
    ...
    gradcam_img = overlay_mask(to_pil_image(input_tensor), to_pil_image(activation_map[0].squeeze(0), mode='F'), alpha=0.5)
    output_path = f"original_xai_imgs/{img_name}"
    await asyncio.to_thread(gradcam_img.save, output_path)
...
async def prediction_prob_and_classLabel(model, selected_model, input_tensor):
    if input_tensor is not None:
        with torch.no_grad():
            output = model(input_tensor.unsqueeze(0))
...
async def get_prediction(selected_model, selected_parameters):
    ...
    threshold = selected_parameters["th"]
    ...
    for image_file in image_files:
        ...
        true_class_label, true_prediction_prob = await prediction_prob_and_classLabel(model, selected_model, input_tensor)
        ...
        await adv_img_save(modified_tensor, f"{image_file}")

        magnitude_change = calculate_magnitude_change(input_tensor, modified_tensor)
        adversary_class_label, adversary_prediction_prob = await prediction_prob_and_classLabel(model, selected_model, modified_tensor)
     
        adv_pred_label.append(adversary_class_label)
        # adv_pred_prob.append(f"{adversary_prediction_prob:.2f}%")
        if adversary_prediction_prob is not None:
            adv_pred_prob.append(f"{adversary_prediction_prob:.2f}%")
        else:
            adv_pred_prob.append("N/A")

        mag_change.append(f"{magnitude_change:.3f}")

        await xai_image_save(model, 'block8', input_tensor, f"{image_file}")
        await xai_adv_image_save(model, 'block8', modified_tensor, f"{image_file}")
